    ...
    private String nextValue(String entityName, int length) {
        return jdbi.inTransaction(handle -> {
            var existing = handle.createQuery(
                            "SELECT last_value FROM onno_sequences WHERE entity_name = :name FOR UPDATE")
            ...
            if (existing.isPresent()) {
                next = existing.get() + 1;
                handle.createUpdate("UPDATE onno_sequences SET last_value = :val WHERE entity_name = :name")
                        .bind("val", next)
                        .bind("name", entityName)
                        .execute();
            } else {
                next = 1;
                handle.createUpdate("INSERT INTO onno_sequences (entity_name, last_value) VALUES (:name, :val)")
                        .bind("name", entityName)
                        .bind("val", next)
                        .execute();
            }
