 ...
 * beans hard-depend on the beans {@code UiAutoConfiguration} only creates when {@code onec.ui
...
class CommentsAutoConfigurationTest {
    ...
    // A MetadataRegistry is always present (OnecAutoConfiguration builds it regardless of the UI
    ...
    void commentsSkipWhenUiBeansAreAbsentSoTheContextStillStarts() {
        // No FieldHintResolver/UiAccessService/etc. on the classpath context — i.e. onec.ui.enabled
    ...
    void commentsStayOffWhenExplicitlyDisabledEvenIfUiBeansExist() {
        runner.withPropertyValues("onec.comments.enabled=false").run(context -> {
            assertThat(context).hasNotFailed();
            assertThat(context).doesNotHaveBean(CommentController.class);
        });
    }
