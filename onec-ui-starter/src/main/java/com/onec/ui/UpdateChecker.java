...
public class UpdateChecker {
    ...
    public void start() {
        if (!props.isEnabled()) {
            log.debug("Update check disabled (onec.ui.update-check.enabled=false)");
            return;
        }
    ...
    void check() {
        try {
            ...
            if (newer) {
                log.info("A newer onec-framework version is available: {} (running {})",
    ...
    private static ThreadFactory threadFactory() {
        return r -> {
            Thread t = new Thread(r, "onec-update-check");
            t.setDaemon(true);
            return t;
        };
    }
