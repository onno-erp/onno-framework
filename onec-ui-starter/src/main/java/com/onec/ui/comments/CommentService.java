...
/**
 * Persistence for {@code onec_comments}: the framework-owned thread store behind the
 * {@code /api/comments} endpoint. It is infrastructure (like {@code onec_schema_history}), not a
...
public class CommentService {
    ...
    static final String TABLE = "onec_comments";
    ...
    private void ensureSchema() {
        jdbi.useHandle(h -> {
            h.execute("CREATE TABLE IF NOT EXISTS " + TABLE + " (\n" +
                    ...
                    ")");
            h.execute("CREATE INDEX IF NOT EXISTS onec_comments_target_idx ON " + TABLE +
