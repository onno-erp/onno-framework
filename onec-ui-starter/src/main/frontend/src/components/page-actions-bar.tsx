...
/**
 * A section of page-level action buttons, bridged from the {@code onec-actions} div-custom block.
 * Each button either runs a server handler (POST /api/divkit/page-action → toast / navigate; any
 * embedded onec-list on the page self-refreshes over SSE from the handler's writes) or routes the
  ...
  const run = (b: PageActionButton) => {
    ...
    api
      ...
      .then((result: ActionResult) => {
        ...
        // so any embedded onec-list on this page reloads its own window.
      })
