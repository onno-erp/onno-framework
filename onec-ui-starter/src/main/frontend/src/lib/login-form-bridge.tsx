...
/**
 * Bridges DivKit's {@code div-custom} block of type {@code onec-login-form} to the React
...
class OnecLoginFormElement extends HTMLElement {
...
export function defineLoginFormElement() {
  ...
  if (!customElements.get("onec-login-form")) {
    customElements.define("onec-login-form", OnecLoginFormElement);
  }
  defined = true;
}
...
export const LOGIN_FORM_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-login-form", { element: "onec-login-form" }],
]);

/** Portals every live {@code <onec-login-form>} to its React form. Mount once, on the login route. */
