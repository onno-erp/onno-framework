...
/**
 * Bridges DivKit's {@code div-custom} block of type {@code onec-comments} to the React
...
class OnecCommentsElement extends HTMLElement {
...
export function defineCommentsElement() {
  ...
  if (!customElements.get("onec-comments")) {
    customElements.define("onec-comments", OnecCommentsElement);
  }
  defined = true;
}
...
export const COMMENTS_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-comments", { element: "onec-comments" }],
]);

/** Portals each mounted {@code onec-comments} element to a React comments panel. */
