...
/**
 * Bridges DivKit's {@code div-custom} block of type {@code onec-actions} to the React
...
class OnecActionsElement extends HTMLElement {
...
export function defineActionsElement() {
  ...
  if (!customElements.get("onec-actions")) {
    customElements.define("onec-actions", OnecActionsElement);
  }
  defined = true;
}
...
export const ACTIONS_BAR_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-actions", { element: "onec-actions" }],
]);

/** Portals every live {@code <onec-actions>} to its button section. Mount once, inside the Router. */
