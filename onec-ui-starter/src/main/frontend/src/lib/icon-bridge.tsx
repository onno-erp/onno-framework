...
/**
 * Bridges DivKit's {@code div-custom} blocks of type {@code onec-icon} to lucide icons,
 * rendered by name. The server (ShellLayoutBuilder) emits nav/account glyphs as
 * {@code onec-icon} nodes carrying {@code name/color/size}; DivKit instantiates the
 * {@link OnecIconElement} custom element and assigns those as properties, and {@link
...
class OnecIconElement extends HTMLElement {
...
export function defineIconElement() {
  ...
  if (!customElements.get("onec-icon")) {
    customElements.define("onec-icon", OnecIconElement);
  }
  defined = true;
}
...
export const ICON_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-icon", { element: "onec-icon" }],
]);

/**
 * Portals every live {@code <onec-icon>} to its lucide icon. Mount once, high in the
