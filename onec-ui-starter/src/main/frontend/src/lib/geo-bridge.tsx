...
/**
 * Bridges DivKit's {@code div-custom} block of type {@code onec-geo} to the React {@link GeoView}.
...
class OnecGeoElement extends HTMLElement {
...
export function defineGeoElement() {
  ...
  if (!customElements.get("onec-geo")) {
    customElements.define("onec-geo", OnecGeoElement);
  }
  defined = true;
}
defineGeoElement();
...
export const GEO_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-geo", { element: "onec-geo" }],
]);

/** Portals every live {@code <onec-geo>} to its React map. Mount once, inside the Router. */
