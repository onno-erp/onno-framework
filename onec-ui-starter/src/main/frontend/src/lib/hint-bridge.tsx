...
/**
 * Bridges DivKit's {@code div-custom} blocks of type {@code onec-hint} to a hoverable "?" help
 * glyph. Raw-DivKit surfaces (the read-only detail view, native count/metric dashboard cards) emit
 * an {@code onec-hint} node carrying {@code text/color/size}; DivKit instantiates the {@link
 * OnecHintElement} custom element and assigns those as properties, and {@link HintPortals} (mounted
...
class OnecHintElement extends HTMLElement {
...
export function defineHintElement() {
  ...
  if (!customElements.get("onec-hint")) {
    customElements.define("onec-hint", OnecHintElement);
  }
  defined = true;
}
...
export const HINT_CUSTOM_COMPONENTS = new Map<string, { element: string }>([
  ["onec-hint", { element: "onec-hint" }],
]);

/**
 * Portals every live {@code <onec-hint>} to its {@link HintIcon}. Mount once, high in the tree
