...
Spring Boot starter that runs a onno web application as a **native desktop app**. The app keeps
...
[`su.onno.desktop`](#packaging-the-suonnodesktop-gradle-plugin) Gradle plugin.

## How it works at runtime

The shell ships generic and dumb. At launch it:

1. boots the embedded JVM (passing `--onno.desktop.home=<os-app-data-dir>`),
...
`onno.desktop.enabled` defaults to `true`, so no config is required:
...
When active (`@ConditionalOnWebApplication` + `onno.desktop.enabled` not `false`),
`OnnoDesktopAutoConfiguration` exposes a `DesktopManifest`, the `DesktopManifestController`, and — if
...
`su.onno.ui.Layout`: fluent top-level setters plus nested lambda builders, producing the immutable
...
|---------|---------|-------|
| `title(...)` | `spring.application.name` (else `onno`) | Window/title-bar text. |
...
`onno.desktop`); everything visual lives in the `DesktopApp` bean above.

| Key | Default | Purpose |
|-----|---------|---------|
| `onno.desktop.enabled` | `true` | Master switch. Set `false` to disable the endpoints and the desktop-mode data/session relocation. |
| `onno.desktop.home` | `""` (empty) | Per-user data home the shell passes at launch via `--onno.desktop.home`. When set, triggers H2 relocation and session persistence (below). Left unset during `bootRun`/dev, so normal runs are untouched. |

## Desktop mode side effects

When `onno.desktop.home` is set (i.e. the shell launched the JVM), a
...
- **Stay logged in.** Sessions are persisted via Spring Session JDBC into the (now file-backed)
  ...
  `classpath:onno-desktop/session/schema-h2.sql`) are added at lowest precedence, so an app can still
  override them.
...
## Packaging: the `su.onno.desktop` Gradle plugin

The sibling **`onno-desktop-gradle-plugin`** (plugin id **`su.onno.desktop`**) turns the Spring
Boot app into a native bundle. In this repo it is wired in via `includeBuild("onno-desktop-gradle-plugin")`
in `settings.gradle.kts`, so `id("su.onno.desktop")` resolves locally without publishing.
...
A bare `id("su.onno.desktop")` already works for an app that uses the Spring Boot plugin —
everything in the `onnoDesktop {}` block has a convention (product name → project name, identifier →
`su.onno.<project>`, version → project version, main jar → `bootJar` output, runtime modules,
Tauri command `["cargo", "tauri"]`, bundle target `app`).

### Tasks (group `onno desktop`)
...
| `desktopStage` | Stage the `bootJar` (as `app/onno.jar`) and the jlinked runtime into the shell as Tauri resources. |
| `desktopIcons` | Generate platform icons from `onnoDesktop.iconSource` (runs only when an icon source is set; `cargo tauri icon`). |
...
- The starter **provides the runtime side**: the manifest/readiness endpoints, the `DesktopApp`
  ...
  `onno-desktop-shell/` so the plugin can extract them for binary-only consumers.
- The plugin **provides the build side**: jlink + shell materialisation + staging the fat jar →
  ...
  `:onno-desktop-starter`'s `src/main/tauri`.
