...
/**
 * When the shell launches the JVM it passes {@code --onno.desktop.home=<dir>}
 ...
 * before {@code spring.datasource.url} is bound. When {@code onno.desktop.home}
...
public class DesktopEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        if (!environment.getProperty("onno.desktop.enabled", Boolean.class, true)) {
            return;
        }
        String home = environment.getProperty("onno.desktop.home", "");
        if (home.isBlank()) {
            return;
        }
        ...
        environment.getPropertySources()
                .addLast(new MapPropertySource("onno-desktop-session", sessionDefaults));
        ...
        if (url.startsWith(H2_FILE_PREFIX)) {
            ...
            environment.getPropertySources()
                    .addFirst(new MapPropertySource("onno-desktop-datasource", overrides));
        }
    }
