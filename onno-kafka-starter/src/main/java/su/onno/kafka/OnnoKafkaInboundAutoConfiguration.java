...
@ConditionalOnProperty(prefix = "onno.kafka.inbound", name = "enabled", havingValue = "true")
@EnableConfigurationProperties(OnnoKafkaProperties.class)
public class OnnoKafkaInboundAutoConfiguration {

    @Bean
    @ConditionalOnBean(DataSource.class)
    @ConditionalOnMissingBean
    public Inbox onnoInbox(DataSource dataSource) {
    ...
    @Bean
    @ConditionalOnMissingBean(name = "onnoInboundContainer")
    public ConcurrentMessageListenerContainer<String, String> onnoInboundContainer(
            KafkaProperties kafkaProperties,
            KafkaEventConsumer consumer,
            OnnoKafkaProperties properties) {

        OnnoKafkaProperties.Inbound inbound = properties.getInbound();
