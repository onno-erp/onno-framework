 ...
 * non-onno producers (whose ids may be arbitrary strings) interoperate without translation.
 */
public class Inbox {
    ...
    public void initSchema() {
        jdbi.useHandle(handle -> handle.execute(
                "CREATE TABLE IF NOT EXISTS onno_inbox (" +
    ...
    public boolean markReceivedIfNew(String id, String eventType, String source) {
        return jdbi.inTransaction(handle -> {
            boolean exists = handle.createQuery("SELECT 1 FROM onno_inbox WHERE _id = :id")
                    .bind("id", id)
                    .mapTo(Integer.class)
                    .findFirst()
                    .isPresent();
            ...
            handle.createUpdate(
                            "INSERT INTO onno_inbox (_id, _source, _event_type, _received_at, _status) " +
    ...
    public void markProcessed(String id) {
        jdbi.useHandle(handle -> handle.createUpdate(
                        "UPDATE onno_inbox SET _status = 'PROCESSED', _processed_at = :processedAt " +
                                "WHERE _id = :id")
                .bind("id", id)
    ...
    public void markFailed(String id, String error) {
        jdbi.useHandle(handle -> handle.createUpdate(
                        "UPDATE onno_inbox SET _status = 'FAILED', _error = :error WHERE _id = :id")
                .bind("id", id)
                .bind("error", error)
                .execute());
    }
