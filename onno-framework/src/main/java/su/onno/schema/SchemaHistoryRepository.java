...
/**
 * Persistence for {@code onno_schema_history}: one row per applied schema change-set
 * (kind {@code SCHEMA}, carrying the metadata snapshot and the DDL that was executed)
 * and one row per executed {@link su.onno.migration.AppMigration} (kind {@code MIGRATION},
...
public class SchemaHistoryRepository {
    ...
    public static final String TABLE = "onno_schema_history";
