...
public class PostgresClusterEventBus implements ClusterEventBus, InitializingBean, DisposableBean {
    ...
    private final ExecutorService listener = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "onec-cluster-listener");
        t.setDaemon(true);
        return t;
    });
    ...
    public PostgresClusterEventBus(DataSource dataSource, ObjectMapper mapper, OnecClusterProperties props) {
    ...
    public void afterPropertiesSet() {
        ...
        log.info("onec-cluster: Postgres LISTEN/NOTIFY bus active on channel '{}' (node {}).", channel, nodeId);
    }
    ...
    public void publish(ClusterEvent event) {
        ...
        try {
            ...
            log.debug("onec-cluster: failed to NOTIFY a {} change; peers may miss this live update: {}",
    ...
    private void listenLoop() {
        ...
        while (running) {
            try (Connection conn = dataSource.getConnection()) {
                ...
                log.debug("onec-cluster: listening on '{}'", channel);
                while (running) {
                ...
                log.warn("onec-cluster: listener connection lost; reconnecting in {} ms ({})", backoff, e.toString());
                sleep(backoff);
                ...
                log.warn("onec-cluster: unexpected listener error; reconnecting in {} ms", backoff, e);
                sleep(backoff);
                backoff = Math.min(backoff * 2, maxBackoffMillis);
            } finally {
        ...
        log.debug("onec-cluster: listener loop stopped");
    }
    ...
    void dispatch(String payload) {
        ...
        try {
            ...
            log.debug("onec-cluster: ignoring unparseable notification payload: {}", e.toString());
            return;
        }
        ...
        for (Consumer<ClusterEvent> sink : sinks) {
            try {
                ...
                log.debug("onec-cluster: sink threw handling a {} event: {}", event.changeType(), e.toString());
            }
        }
    }
    ...
    static String serialize(ObjectMapper mapper, ClusterEvent event, int maxPayloadBytes) {
        try {
            ...
            if (utf8Length(json) <= maxPayloadBytes) {
                log.debug("onec-cluster: dropped oversized naturalKey from a {} event on {}",
            ...
            if (utf8Length(json) <= maxPayloadBytes) {
                log.debug("onec-cluster: degraded an oversized {} event to a coarse '*' notice", event.changeType());
                return json;
            }
            log.debug("onec-cluster: dropping a {} event; even the coarse envelope exceeds {} bytes",
                    event.changeType(), maxPayloadBytes);
            return null;
        } catch (Exception e) {
            log.debug("onec-cluster: failed to serialize a cluster event: {}", e.toString());
            return null;
        }
    }
    ...
    private static String safeChannel(String channel) {
        ...
        log.warn("onec-cluster: invalid channel '{}'; falling back to 'onec_cluster_events'", channel);
        return "onec_cluster_events";
    }
