package com.onec.desktop.gradle
...
/**
 * User-facing configuration for [DesktopPlugin], exposed as the `onecDesktop {}`
 * block. Everything has a sensible convention so a bare `id("com.onec.desktop")`
...
abstract class DesktopExtension {
    ...
    /** Reverse-DNS bundle identifier. Defaults to `com.onec.<project>`. */
    ...
    /**
     * The Tauri shell sources. Defaults to the `:onec-desktop-starter` project's
...
/**
 * Adds a `packageDesktop` task that turns an onec Spring Boot app into a native
...
class DesktopPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        val ext = project.extensions.create("onecDesktop", DesktopExtension::class.java)

        ext.productName.convention(project.provider { project.name })
        ext.identifier.convention(project.provider { "com.onec.${project.name}" })
        ...
        val starterProject = project.rootProject.findProject(":onec-desktop-starter")
        ...
        val embeddedShell = if (starterProject == null) {
            project.configurations.create("onecDesktopShell") {
                isCanBeConsumed = false
                isCanBeResolved = true
                defaultDependencies {
                    add(project.dependencies.create("com.onec:onec-desktop-starter:$STARTER_VERSION"))
                }
            }
        } else {
            null
        ...
        val workDir = project.layout.buildDirectory.dir("onec-desktop")
        ...
        val extractDesktopShell = project.tasks.register("extractDesktopShell", Copy::class.java) {
            group = GROUP
            description = "Extract the generic Tauri shell from the onec desktop starter artifact."
        ...
        val desktopShell = project.tasks.register("desktopShell", Copy::class.java) {
            ...
            doLast {
                ...
                bundle["resources"] = listOf("runtime/**/*", "app/onec.jar")
        ...
        val desktopIcons = project.tasks.register("desktopIcons", Exec::class.java) {
            group = GROUP
            description = "Generate platform icons from onecDesktop.iconSource."
    ...
    companion object {
        private const val GROUP = "onec desktop"
