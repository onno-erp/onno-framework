// onec desktop shell.
//
// Generic across every onec application: it boots the bundled JVM (a jlinked
...
/// Ask the OS for a free TCP port so multiple onec apps never collide.
fn free_port() -> u16 {
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // by the `com.onec.desktop` Gradle task and declared as resources.
            let java = resource_dir.join("runtime").join(if cfg!(windows) {
                "bin/java.exe"
            } else {
                "bin/java"
            });
            let jar = resource_dir.join("app").join("onec.jar");
            ...
            let child = Command::new(&java)
                ...
                .spawn()
                .expect("failed to launch embedded onec server");
        ...
        .expect("error while building the onec desktop shell")
        .run(|app, event| {
