...
@ConfigurationProperties(prefix = "onno.auth")
public class OnnoAuthProperties {
     ...
     *
     * <ul>
     *   <li>{@link Mode#IN_MEMORY} (default) — username/password against {@code onno.auth.users},
     ...
     * {@code onno.auth.users[*]}. Production deployments should disable in-memory users
                        ...
                        if (roles.isClientRoles()) {
                            if (isBlank(roles.getClientId())) {
                                throw new IllegalStateException(
                                        "onno.auth.oidc.roles.client-roles is enabled but client-id is not set");
                            }
     ...
     * {@code onno.auth.oidc.roles.sources[*]} entries.
     */
