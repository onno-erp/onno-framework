...
| `onno-ui-starter` | `su.onno` | Generic REST controllers under `/api/**`, the DivKit server-driven UI layer, the bundled React/Vite SPA, media uploads, SSE event stream, and generic server/browser UI-feature extension hosts. |
| `onno-collaboration-starter` | `su.onno` | Opt-in comments, mentions, notifications, live presence, and an automatically discovered React UI feature module. |
...
The same classpath plugin channel carries **UI feature packs**. A trusted starter can register
custom DivKit blocks, global roots, tab/row/sidebar adornments, and `onno://` actions through
`registerUiFeature`; server-side `EntityDetailUiContributor` and `ShellUiContributor` beans add the
corresponding metadata. `onno-collaboration-starter` is the reference implementation. With that
artifact absent, neither its server APIs nor its React implementation ship.
...
but other modules emit their own. Comment-thread posts and deletes (`onno-collaboration-starter`; not a modelled
