...
| Mentions | `GET /api/mentions?q=` — cross-entity `@`-mention typeahead over readable catalogs/documents; comment bodies carry mentions as `@[Display](kind/name/id)` tokens resolved live, and each readable mention publishes `EntityMentionedEvent` (consumed by the notification inbox; further delivery additive via `@EventListener`) (ui-starter) |
| Notifications | `GET /api/notifications[?unread=true]`, `GET /api/notifications/unread-count`, `POST /api/notifications/{id}/read`, `POST /api/notifications/read-all`, `DELETE /api/notifications/{id}` — per-user inbox behind the bell widget; addressed by recipient key (`user:`/`role:`/`record:`), every op scoped to the caller; produced server-side via `NotificationService` (+ mention/posting bridges), stored in `onec_notifications` (ui-starter) |
