...
class CrmWorkspaceAccessTest {
    @Test void agentsCannotManageChannelCredentials() {
        var access=mock(UiAccessService.class);var provider=mock(CrmChannelConnection.class);
        Principal agent=()->"agent";
        var command=new CrmChannelController.Command();command.action="reconnect";command.credential="secret";
        assertThatThrownBy(()->new CrmChannelController(List.of(provider),access).command("telegram",command,agent))
    ...
    @Test void managerCanInvokeOnlySupportedConnectorActions() {
        var access=mock(UiAccessService.class);var provider=mock(CrmChannelConnection.class);
        Principal manager=()->"manager";when(access.hasAnyRole(manager,List.of("CRM_MANAGER"))).thenReturn(true);
        ...
        var controller=new CrmChannelController(List.of(provider),access);var command=new CrmChannelController.Command();command.action="check";
