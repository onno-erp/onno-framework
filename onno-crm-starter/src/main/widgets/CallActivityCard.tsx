...
export function callActivity(body: string) {
  ...
  const titles: Record<string,string> = {
    "Call completed":"Phone call", "Call planned":"Upcoming call",
    "Meeting planned":"Scheduled meeting", "Activity":"Meeting summary",
  };
...
export function CallActivityCard({activity,author,at}: {activity:NonNullable<ReturnType<typeof callActivity>>;author:string;at?:string}) {
  ...
  return <article aria-label={activity.title} className="mx-auto w-full max-w-lg overflow-hidden rounded-panel border border-border bg-card text-foreground shadow-sm">
    <div className="flex items-center gap-3 border-b border-border/60 px-4 py-3">
      ...
      <span className="rounded-pill bg-muted px-2 py-1 text-[10px] text-muted-foreground">{activity.planned?"Planned":"Logged"}</span>
    </div>
    <div className="space-y-2 px-4 py-3">
      {activity.scheduled&&<p className="text-xs font-medium">{activity.scheduled}</p>}
      <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Summary</p>
      ...
      {long&&<Button size="sm" variant="ghost" aria-expanded={expanded} onClick={()=>setExpanded(value=>!value)}>{expanded?"Show less":"Read full summary"}</Button>}
    </div>
    <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/60 px-4 py-2.5">
      <span className="text-[10px] text-muted-foreground">Internal · visible to your team</span>
      {activity.recording&&<a href={activity.recording} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-field px-2 py-1 text-xs font-medium text-primary hover:bg-primary/10"><ExternalLink className="size-3.5" />Open recording</a>}
    </div>
  </article>;
}
