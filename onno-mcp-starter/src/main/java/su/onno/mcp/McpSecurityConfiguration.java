 ...
 * (e.g. from {@code onno-auth-starter}), so MCP callers map onto the same users and roles
 * the rest of the system enforces.
 *
 * <p>The chain is {@code securityMatcher}-scoped to {@code onno.mcp.endpoint} and ordered
...
public class McpSecurityConfiguration {
    ...
    @Bean
    @Order(1)
    public SecurityFilterChain onnoMcpSecurityFilterChain(HttpSecurity http, OnnoMcpProperties properties)
            throws Exception {
