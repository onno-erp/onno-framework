...
[onec-enterprise](https://github.com/onec-erp/onec-enterprise) repo (Maven group
`com.onec.enterprise`, onec Commercial License). Today: `onec-guesty-starter` (Guesty short-term
...
The only framework types they touch are `com.onec.types.Ref` and `com.onec.types.RefResolver`. The
...
Auto-configuration is gated by `@ConditionalOnProperty(prefix = "onec.<name>", name = "enabled")`
...
`io.github.onec-erp:onec-framework` (`api`) + `onec-framework-starter` (`implementation`), and
publish to the private `onec-enterprise` GitHub Packages repo.
...
`onec-hospedajes-starter` keeps an audit ledger (`HospedajesCommunicationLog`, JDBI, table
`onec_hospedajes_comunicacion`) tracking SUBMITTED → REGISTERED|REJECTED → CANCELLED keyed by
...
`@ConditionalOnBean(DataSource.class)`, then creates its schema with raw JDBI DDL (`onec_`-prefixed
...
- **One switch gates both sides.** The host integration `@Configuration` reuses the same
  `@ConditionalOnProperty(prefix = "onec.<name>", name = "enabled")` as the starter, so one flag
  turns on the connector and its host wiring together.

## When editing onec-enterprise, keep its docs in sync too
...
`onec-tochka-starter`, and that module still references the old `com.onec:*` core coordinates instead
of `io.github.onec-erp:*`.)
