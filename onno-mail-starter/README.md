...
Spring Boot starter for transactional email in a onno application. It renders Thymeleaf templates
...
standard `spring.mail.*` keys**, not under `onno.mail.*`. The `onno.mail.*` namespace only governs
...
`onno.mail.enabled` is not `false` (**default `true` — enabled out of the box**). Point the SMTP
...
onno:
  mail:
    default-from: "onno <no-reply@example.com>"
```
...
| Key | Default | Purpose |
|-----|---------|---------|
| `onno.mail.enabled` | `true` | Master switch for the auto-configuration. |
| `onno.mail.provider` | `smtp` | Active dispatcher, chosen by its `name()`: `smtp`, `http`, `log`, `file`, or `failover`. |
| `onno.mail.default-from` | — | From: address used when a `MailMessage` doesn't set one. SMTP send fails if neither is set. |
| `onno.mail.base-packages` | app's auto-config packages | Packages scanned for `@MailTemplate`. |
| `onno.mail.use-outbox` | `true` | When true, the template `send(...)` overloads queue via the outbox (async); when false they dispatch synchronously. |
| `onno.mail.relay-batch-size` | `50` | Messages claimed per relay run. |
| `onno.mail.encoding` | `UTF-8` | Charset for rendering and MIME. |
| `onno.mail.derive-plain-text` | `true` | When a template is HTML-only, derive a plain-text alternative so the mail is multipart. |
| `onno.mail.relay.enabled` | `true` | Scheduled outbox relay (needs an outbox / `DataSource`). |
| `onno.mail.relay.interval-ms` | `30000` | Delay between relay runs. |
| `onno.mail.relay.max-attempts` | `5` | Delivery attempts before a message is marked `FAILED`. |
| `onno.mail.relay.lease-timeout-ms` | `300000` | How long a claimed (`SENDING`) message may sit before another worker reclaims it. |
| `onno.mail.file.directory` | `build/mail` | Output dir for the `file` dev dispatcher (`.eml` files). |
| `onno.mail.http.url` | — | Endpoint the `http` provider POSTs to. |
| `onno.mail.http.method` | `POST` | HTTP method for the `http` provider. |
| `onno.mail.http.headers` | `{}` | Static headers per request (e.g. `Authorization: Bearer ...`). |
| `onno.mail.http.body-template` | — | Thymeleaf (TEXT-mode) template that renders the provider's JSON body. `MailMessage` is `msg`. |
| `onno.mail.http.success-status-max` | `299` | Highest HTTP status still treated as success. |
| `onno.mail.failover.providers` | `[]` | Ordered provider names tried in turn when `provider=failover`. Required in that mode. |
| `onno.mail.preview.enabled` | `false` | Dev-only template-preview endpoints. Never enable in production. |
| `onno.mail.preview.path` | `/onno/mail/preview` | Base path for the preview endpoints. |
| `onno.mail.webhook.enabled` | `false` | Inbound delivery-event webhook that feeds the suppression list. |
| `onno.mail.webhook.path` | `/onno/mail/events` | Path the provider posts events to. |
...
class Notifications {
    ...
    void confirm(Booking booking) {
        ...
        // Routed via the outbox (async) or synchronously per onno.mail.use-outbox.
...
The active provider is `onno.mail.provider`. All dispatchers are wired as candidates and the named one
is selected:

- **`smtp`** — Spring's `JavaMailSender`. Multipart when HTML + text or attachments are present.
- **`http`** — universal REST adapter: POSTs a Thymeleaf-rendered JSON body (`onno.mail.http.body-template`)
  to `onno.mail.http.url`, so a SendGrid/SES/etc. API is onboarded with config only. Requires `spring-web`
...
- **`file`** — writes each message as an `.eml` file under `onno.mail.file.directory` (dev/preview).
- **`failover`** — tries `onno.mail.failover.providers` in order, advancing only on failure; throws only
...
`onno_mail_outbox` and `onno_mail_suppression`, via JDBI.

- **Outbox.** `queue(...)` serialises the message to `onno_mail_outbox`. The scheduled relay claims due
...
- **Suppression list.** Recipients on `MailSuppressionList` (hard bounces, complaints, unsubscribes) are
  ...
  webhook (`onno.mail.webhook.enabled=true`) — translate your provider's events to the neutral shape
...
- **SMTP settings are Spring's, not ours.** Host, port, username, password, auth and STARTTLS/TLS live
  under `spring.mail.*` (e.g. `spring.mail.properties.mail.smtp.starttls.enable: true`). `onno.mail.*`
  does not configure the transport.
- **From address is mandatory for SMTP.** If a message has no `from` and `onno.mail.default-from` is
...
- **`queue(...)` needs the outbox.** Calling it without a `DataSource` throws `IllegalStateException`.
  Either configure a `DataSource` or set `onno.mail.use-outbox=false` to dispatch synchronously.
- **Queued mail needs the relay running.** With `onno.mail.relay.enabled=false` (or no scheduling),
...
- **Preview is dev-only.** It instantiates a sample target via its no-arg constructor and renders
  templates over HTTP — never enable `onno.mail.preview.enabled` in production.
