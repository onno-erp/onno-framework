...
/**
 * Side-effect queue for outbound mail. Lives in its own table {@code onno_mail_outbox}
 * rather than the framework's domain-event outbox: mail is a command-to-execute, not an event-to-broadcast,
 * and giving it a separate table avoids racing with the Kafka relay over the shared {@code onno_outbox}.
 *
...
public class MailOutbox {
    ...
    private static final String DDL =
            "CREATE TABLE IF NOT EXISTS onno_mail_outbox (\n" +
    ...
    private static final String DDL_IDEMPOTENCY_INDEX =
            "CREATE UNIQUE INDEX IF NOT EXISTS onno_mail_outbox_idem " +
                    "ON onno_mail_outbox (_idempotency_key)";

    // Brings tables created before the claim-based relay up to date.
    private static final String DDL_CLAIMED_AT =
            "ALTER TABLE onno_mail_outbox ADD COLUMN IF NOT EXISTS _claimed_at TIMESTAMP";
    ...
    public UUID enqueue(String payload, String provider, String idempotencyKey) {
        ...
        String insert = "INSERT INTO onno_mail_outbox " +
    ...
    private Optional<UUID> findByIdempotencyKey(String idempotencyKey) {
        return jdbi.withHandle(h -> h.createQuery(
                        "SELECT _id FROM onno_mail_outbox WHERE _idempotency_key = :key")
    ...
    public List<Pending> claimBatch(int limit, Duration leaseTimeout) {
        ...
        if (dialect() == SqlDialect.POSTGRESQL) {
            return jdbi.withHandle(h -> h.createQuery(
                            "UPDATE onno_mail_outbox SET _status = 'SENDING', _claimed_at = :now " +
                                    "WHERE _id IN (" +
                                    "  SELECT _id FROM onno_mail_outbox " + DUE_WHERE +
        ...
        return jdbi.inTransaction(h -> {
            List<Pending> due = h.createQuery(
                            "SELECT _id, _payload, _attempts FROM onno_mail_outbox " + DUE_WHERE +
                                    "FOR UPDATE")
                    .bind("now", now)
                    .bind("staleBefore", staleBefore)
                    .bind("limit", limit)
            ...
            h.createUpdate("UPDATE onno_mail_outbox SET _status = 'SENDING', _claimed_at = :now " +
                            "WHERE _id IN (<ids>)")
                    .bind("now", now)
    ...
    public void markDispatched(UUID id) {
        jdbi.useHandle(h -> h.createUpdate(
                        "UPDATE onno_mail_outbox SET _status = 'SENT', _dispatched_at = :now " +
                                "WHERE _id = :id")
                .bind("id", id)
    ...
    public void recordFailure(UUID id, int attempts, String error, LocalDateTime nextAttempt, boolean exhausted) {
        jdbi.useHandle(h -> h.createUpdate(
                        "UPDATE onno_mail_outbox SET _attempts = :attempts, _last_error = :err, " +
