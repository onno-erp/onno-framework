 ...
 *
 * <p>The JSON payload is produced by the Thymeleaf template at {@code onno.mail.http.body-template}
...
public class HttpMailDispatcher implements MailDispatcher {
    ...
    public void dispatch(MailMessage message) {
        ...
        if (cfg.getUrl() == null || cfg.getUrl().isBlank()) {
            throw new MailDeliveryException("onno.mail.http.url is not configured");
        }
        if (cfg.getBodyTemplate() == null || cfg.getBodyTemplate().isBlank()) {
            throw new MailDeliveryException("onno.mail.http.body-template is not configured");
        }
