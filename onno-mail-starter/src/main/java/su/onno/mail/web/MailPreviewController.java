package su.onno.mail.web;

import su.onno.mail.template.MailRenderer;
import su.onno.mail.template.MailTemplateDescriptor;
...
/**
 * Dev-only endpoints (enabled via {@code onno.mail.preview.enabled=true}) for inspecting templates:
...
public class MailPreviewController {
    ...
    @GetMapping(path = "${onno.mail.preview.path:/onno/mail/preview}", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @GetMapping(path = "${onno.mail.preview.path:/onno/mail/preview}/{name}")
