...
abstract class MailOutboxContract {
    ...
    void setUpOutbox() {
        ...
        outbox.initSchema();
        jdbi.useHandle(h -> h.execute("DELETE FROM onno_mail_outbox"));
    }
    ...
    void staleSendingRowIsReclaimedAfterLeaseExpiry() {
        ...
        jdbi.useHandle(h -> h.createUpdate(
                        "UPDATE onno_mail_outbox SET _claimed_at = :stale WHERE _id = :id")
    ...
    private int rowCount() {
        return jdbi.withHandle(h -> h.createQuery("SELECT COUNT(*) FROM onno_mail_outbox")
                .mapTo(Integer.class).one());
    }

    private String status(UUID id) {
        return jdbi.withHandle(h -> h.createQuery(
                        "SELECT _status FROM onno_mail_outbox WHERE _id = :id")
