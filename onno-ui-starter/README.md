...
| `presets`, `default` | timeRange | The shared picker's quick-picks and starting window. `presets` is a comma list of duration ids (`<n><unit>` where `s`/`m`/`h`/`d`/`w`/`M`/`y` are second…year — note `m`=minute, `M`=month — plus `all`), e.g. `15m,1h,24h,7d,30d,90d,1y,all`. `default` names one of them (e.g. `30d`). Omit both for the built-in ladder defaulting to the last 30 days. A user's saved selection wins over `default` when this dashboard's `presets` offer it (an absolute window always wins); a saved relative range outside the ladder — a `1h` carried over from another board onto a `30d,90d,1y` one — would apply invisibly, so it falls back to `default` and that becomes the saved selection. The picker footer also shows the automatic chart interval (`Auto (1 min)`, etc.) and lets the viewer override it globally for auto-bucketed charts; charts authored with `bucketMode=fixed` remain fixed. |
