...
/**
 * Resolves users' avatar image URLs, live, from the identity catalog — the one the login
 ...
 * avatar-hinted attribute, or no stored value, it returns nothing and callers fall back to initials.
 * Resolving live keeps task and feature-pack surfaces aligned with the current identity record.
 */
public class UserAvatarResolver {
    ...
    public UserAvatarResolver(UiLayout layout, MetadataRegistry registry,
    ...
    public Map<String, String> avatarsFor(Collection<String> authorIds) {
        ...
        List<UUID> ids = authorIds.stream()
                ...
                .distinct()
                .map(UserAvatarResolver::parseUuid)
                .filter(Objects::nonNull)
                .toList();
        if (ids.isEmpty()) {
            return Map.of();
