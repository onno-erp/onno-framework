...
describe("presence store", () => {
  it("seeds from the snapshot, applies live deltas, excludes self, and aggregates by entity", async () => {
    ...
    // Snapshot seeds r1; the caller (me) is filtered out, leaving Ada.
    const record = renderHook(() => useRecordViewers("r1"));
    await waitFor(() => expect(record.result.current.map((v) => v.userId)).toEqual(["u2"]));
    ...
    // Sidebar aggregation unions the distinct other viewers across every property record.
    ...
    // The by-id map (list rows) carries both records, self still excluded.
    const byId = renderHook(() => useViewersById());
    expect(byId.result.current.get("r1")?.map((v) => v.userId)).toEqual(["u2"]);
